...
let test_constants () =
  ...
  group "pointer null"; begin
    (* xCHECK: const_pointer_null = global ptr null
     *)
...
let test_aliases () =
  (* xCHECK: @alias = alias i32, ptr @aliasee
   *)
  let forty_two32 = const_int i32_type 42 in
...
let test_builder () =
  ...
  group "casts"; begin
     ...
     * xCHECK-DAG: %build_inttoptr = inttoptr i32 %P1 to ptr
     * xCHECK-DAG: %build_ptrtoint = ptrtoint ptr %build_inttoptr to i64
     * xCHECK-DAG: %build_ptrtoint2 = ptrtoint ptr %build_inttoptr to i64
     * xCHECK-DAG: %build_bitcast = bitcast i64 %build_ptrtoint to double
     * xCHECK-DAG: %build_bitcast2 = bitcast i64 %build_ptrtoint to double
  ...
  group "comparisons"; begin
     ...
     * xCHECK: %build_is_not_null = icmp ne ptr %X1, null
     * CHECK: %build_ptrdiff
     *)
  ...
  group "miscellaneous"; begin
     ...
     * xCHECK: %build_va_arg = va_arg ptr null, i32
  ...
  group "indirectbr"; begin
    (* xCHECK: indirectbr ptr blockaddress(@X7, %IBRBlock2), [label %IBRBlock2, label %IBRBlock3]
     *)
  ...
  group "memory"; begin
     ...
     * xCHECK: %build_load = load volatile i32, ptr %build_array_alloca, align 4
     * xCHECK: store volatile i32 %P2, ptr %build_alloca, align 4
     * xCHECK: %build_gep = getelementptr i32, ptr %build_array_alloca, i32 %P2
     * xCHECK: %build_in_bounds_gep = getelementptr inbounds i32, ptr %build_array_alloca, i32 %P2
     * CHECK: %build_struct_gep = getelementptr inbounds{{.*}}%build_alloca2, i32 0, i32 1
     * xCHECK: %build_atomicrmw = atomicrmw xchg ptr %p, i8 42 seq_cst
     *)
