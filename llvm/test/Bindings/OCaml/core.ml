...
let test_constants () =
  ...
  (* Error: Unbound value aggregate_element *)
  ...
  ignore (define_global "const_ptrtoint" (const_ptrtoint
    (const_gep i8_type (const_null (pointer_type2 context))
...
let test_builder () =
  ...
  group "comparisons"; begin
    ...
    let p0 = build_load (pointer_type2 context) g0 "X0" atentry in
    let p1 = build_load (pointer_type2 context) g1 "X1" atentry in
    ignore (build_is_null p0 "build_is_null" atentry);
    ignore (build_is_not_null p1 "build_is_not_null" atentry);
    (* FIXME ignore (build_ptrdiff i8_type p1 p0 "build_ptrdiff" atentry); *)
  end;
